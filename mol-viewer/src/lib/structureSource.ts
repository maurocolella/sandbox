/*
 ...
 URLs and paths pass through unchanged.
...
export function resolveStructureSource(input: string): StructureSource | null {
  ...
  if (CLASSIC_ID.test(text)) {
    ...
    return { url: `https://files.rcsb.org/download/${id}.cif`, pdbId: id };
  }
  if (EXTENDED_ID.test(text)) {
    ...
    return { url: `https://files.rcsb.org/download/${classic ?? id}.cif`, pdbId: classic ?? id };
  }
